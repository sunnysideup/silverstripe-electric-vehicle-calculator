...
return!1})},transitionFromStartupToResults:function(){jQuery("body").addClass("loading");var b=jQuery('input[name="startup-car-value"]').val(),b=parseFloat(b.replace(/[^0-9.]/g,"")),c=jQuery('input[name="petrol-spent-per-week"]').val(),c=parseFloat(c.replace(/[^0-9.]/g,"")),c=c/EVC.DefaultData.costOfPetrolPerLitre*EVC.DefaultData.fuelEfficiencyCV/7,d=!0;0===parseInt(c)&&(jQuery('input[name="petrol-spent-per-week"]').focus(),d=!1);0===parseInt(b)&&jQuery('input[name="startup-car-value"]').focus();
d?(EVC.HTMLInteraction.setValue("CVValueToday",b),EVC.HTMLInteraction.setValue("kmDrivenPerDay",c),jQuery("#start-up-screen").slideUp(function(){jQuery("#ElectricVehicleCalculator").slideDown(function(){window.setTimeout(function(){jQuery("#CVValueTodayField").change();jQuery("body").removeClass("loading");jQuery("#ProfitAndLoss").addClass("full-screen")},500);jQuery("#ViewDetailsNow a").click(function(a){EVC.HTMLInteraction.transitionFromResultsToDetails();return!1})})})):(alert("Please answer both questions."),
jQuery("body").removeClass("loading"));return!1},transitionFromResultsToDetails:function(){jQuery("body").addClass("results-ready");jQuery("#ViewDetailsNow").remove();jQuery("#ProfitAndLoss").removeClass("full-screen");return!1},selectFirstInput:function(){window.setTimeout(function(){jQuery("#CVValueTodayDisplay").click()},300)},makeGraphClickable:function(){jQuery("canvas").on("click",function(b){b=EVC.graphMaker.currentGraph.getPointsAtEvent(b);b=parseFloat(b[0].label.replace(/\$|,/g,""));var c=
jQuery(this).attr("data-for");jQuery("#"+c+"Field").val(b).change()})},createFormFieldsFromList:function(b){var c="",d="";this.isTouchScreenTest()&&(d=' readonly="readonly" ');for(var a in b)if(b.hasOwnProperty(a)){var e=b[a],f=EVC.DataDescription.labels[a],g=EVC.DataDescription.desc[a],k=a+"Holder",h=a+"Field",m=a+"Display",n=a+"FieldRange",l=EVC.HTMLInteraction.getValueFromDefaultsOrSession(a,!1),p=EVC.HTMLInteraction.getValueFromDefaultsOrSession(a,!0),q=EVC.DefaultDataMinMax[a][0],r=EVC.DefaultDataMinMax[a][1],
t=Math.round((r-q)/EVC.numberOfSteps*100)/100,u='step="'+t+'"',c=c+('<div id="'+k+'" class="fieldHolder '+e+'">'),c=c+('<p id="'+a+'Modal" class="modalP"><a href="#'+k+'" class="closeModal" onclick="return EVC.HTMLInteraction.closeModal(\''+a+"');\">close</a></p>"),c=c+('<label for="'+n+'">'),c=c+("<strong onclick=\"return EVC.HTMLInteraction.showDesc('"+a+"');\">"+f+"</strong>"),c=c+('<span class="desc">'+g+"</span>"),c=c+"</label>",c=c+'<div class="middleColumn">',c=c+('<a href="#'+k+'" class="displayValue" id="'+
m+'" onclick="return EVC.HTMLInteraction.showDesc(\''+a+"');\">"+p+"</a>");EVC.HTMLInteraction.hasRangeSlider&&(c+='<input type="range" tabindex="-1" class="'+e+'" id="'+n+'" oninput="return  EVC.HTMLInteraction.showUpdatedValue(\''+a+"', this);\" onchange=\"return EVC.HTMLInteraction.setValue('"+a+'\', this);" value="'+l+'"  min="'+q+'" max="'+r+'" step="'+t+'" />');c+='<input type="number" inputmode="numeric" pattern="[0-9]*"  class="'+e+'" oninput="return  EVC.HTMLInteraction.startInput(\''+a+
'\', this);" id="'+h+'" onclick="return EVC.HTMLInteraction.clickInput(\''+a+"', this);\" onfocus=\"return EVC.HTMLInteraction.inputReady('"+a+"', this);\" onchange=\"return EVC.HTMLInteraction.setValue('"+a+'\', this);" value="'+l+'" '+d+' min="'+q+'" max="'+r+'" '+u+" />";c+="</div>";EVC.HTMLInteraction.hasGraphs&&(c+='<p class="graphShower"><a href="#'+a+'Chart" onclick="return EVC.HTMLInteraction.showGraphNow(\''+a+"')\">view influence graph \u21e3</a></p>",c+='<figure id="'+a+'Chart" class="chartHolder"><figcaption></figcaption><canvas data-for="'+
...
jQuery("#ProfitAndLoss p.warning").show())},updateInProgress:!1,setValue:function(b,c,d){!0!==d&&(d=!1);this.updateInProgress?console.error("Application can not be updated right now."):(jQuery("body").addClass("loading"),delay=0,jQuery("body").hasClass("results-ready")&&(delay=50),window.setTimeout(function(){this.updateInProgress=!0;var a=b+"Field",e=b+"FieldRange",f=b+"Display",g="#"+b+"Holder",k="#"+b+"Holder label strong",h=EVC.DefaultData[b],m="",n=!1;if(isNaN(c)){var n=EVC.isChanged=!0,l=jQuery(c).val(),
l=parseFloat(l.replace(/\$|,/g,""));isNaN(l)?(l=EVC.ActualData[b],isNaN(l)&&(l=h)):m=jQuery(c).attr("id")}else d&&(n=EVC.isChanged=!0),l=c;var l=Math.round(100*l)/100,p=EVC.DataDescription.labels[b];l!=h?(0<h?jQuery(k).text(p+" (default = "+EVC.HTMLInteraction.formatValue(b,h)+")"):jQuery(k).text(p),jQuery(g).addClass("changed")):(jQuery(k).text(p),jQuery(g).removeClass("changed"));"kmDrivenPerDay"==b&&3<l/365&&(l=Math.round(l/365),m="");a!=m&&jQuery("#"+a).val(l);e!=m&&jQuery("#"+e).val(l);a=EVC.HTMLInteraction.formatValue(b,
l);jQuery("#"+f).text(a);EVC.serverInteraction.workableLinks()&&jQuery.ajax({method:"GET",url:EVC.serverInteraction.saveLink(),data:{key:b,value:l},cache:!1}).done(function(a){a&&(EVC.isLocked=!1,EVC.serverKey=a)}).fail(function(a,b){alert("Data could not be saved: "+b)});EVC.ActualData[b]=l;if("kmDrivenPerDay"==b||"yearsBeforeSwitch"==b||"yearsAfterSwitch"==b)"kmDrivenPerDay"==b?EVC.myData.updateKmDrivenPerDay(l):"yearsBeforeSwitch"==b?EVC.myData.updateYearsBeforeSwitch(l):"yearsAfterSwitch"==b&&
EVC.myData.updateYearsAfterSwitch(l);n&&EVC.HTMLInteraction.updateScreen(b);EVC.HTMLInteraction.updateInProgress=!1;delay&&jQuery("body").removeClass("loading")},delay));return!1},showDesc:function(b,c){var d=jQuery("div#"+b+"Holder");jQuery(".infocus").each(function(a,b){jQuery(d).attr("id")!=jQuery(b).attr("id")&&jQuery(b).removeClass("infocus").find("figure").hide()});jQuery(d).hasClass("infocus")||(d.addClass("infocus"),c||jQuery("html, body").animate({scrollTop:jQuery("#"+b+"Holder").offset().top-
(jQuery("#ProfitAndLoss").height()+20)},500,function(){jQuery("#"+b+"Holder").find('input[type="number"]').focus()}));return!1},showGraphNow:function(b){jQuery("#"+b+"Chart").show("slow",function(){EVC.graphMaker.makeGraph(b)}).prev(".graphShower").hide();return!1},updateScreen:function(b){jQuery("body").addClass("loading");EVC.debug&&console.debug("== UPDATING SCREEN ==");this.populateResultTable();this.populateCalculations();this.updateLinks();EVC.scenarios.checkInfluence();EVC.isReadyToCalculate()?
jQuery("body").removeClass("not-ready").addClass("ready"):jQuery("body").removeClass("ready").addClass("not-ready");jQuery("#ProfitAndLoss").addClass("fixed");this.isTouchScreenTest()&&(jQuery("#ProfitAndLoss p.good").show(),jQuery("#ProfitAndLoss p.warning").hide());"undefined"!==typeof b&&EVC.graphMaker.makeGraph(b);jQuery("body").removeClass("loading")},setMyValue:function(b,c){var d=c.value,d=parseFloat(d.replace(/\$|,/g,"")),d=this.formatValue(b,d);c.value=d},resetSession:function(){alert("to be completed")},
isScrolledIntoView:function(b){b=jQuery(b);var c=jQuery(window),d=c.scrollTop(),c=d+c.height(),a=b.offset().top;return a+b.height()<=c&&a>=d},openModal:function(b,c){this.showDesc(b,!0);jQuery("body").addClass("modalMode");jQuery("#"+b+"Holder").addClass("modal");return!1},closeModal:function(b){jQuery("body").removeClass("modalMode");jQuery("#"+b+"Holder").removeClass("modal").removeClass("infocus");return!1},inlineExpandListener:function(){jQuery(".inline-expand").on("click",function(b){b.preventDefault();
b=jQuery(this).attr("href");b=b.substring(b.indexOf("#"));jQuery(b).toggle("slow");return!1})}};
