...
jQuery("body").removeClass("loading"));return!1},transitionFromResultsToDetails:function(){jQuery("#ViewDetailsNow").remove();jQuery("#ProfitAndLoss").removeClass("full-screen");return!1},selectFirstInput:function(){window.setTimeout(function(){jQuery("#CVValueTodayDisplay").click()},300)},makeGraphClickable:function(){jQuery("canvas").on("click",function(b){b=EVC.graphMaker.currentGraph.getPointsAtEvent(b);b=parseFloat(b[0].label.replace(/\$|,/g,""));var c=jQuery(this).attr("data-for");jQuery("#"+
c+"Field").val(b).change()})},createFormFieldsFromList:function(b){var c="",d="";this.isTouchScreenTest()&&(d=' readonly="readonly" ');for(var a in b)if(b.hasOwnProperty(a)){var e=b[a],f=EVC.DataDescription.labels[a],g=EVC.DataDescription.desc[a],k=a+"Holder",h=a+"Field",l=a+"Display",m=a+"FieldRange",q=EVC.HTMLInteraction.getValueFromDefaultsOrSession(a,!1),t=EVC.HTMLInteraction.getValueFromDefaultsOrSession(a,!0),n=EVC.DefaultDataMinMax[a][0],p=EVC.DefaultDataMinMax[a][1],r=Math.round((p-n)/EVC.numberOfSteps*
100)/100,u='step="'+r+'"',c=c+('<div id="'+k+'" class="fieldHolder '+e+'">'),c=c+('<p id="'+a+'Modal" class="modalP"><a href="#'+k+'" class="closeModal" onclick="return EVC.HTMLInteraction.closeModal(\''+a+"');\">close</a></p>"),c=c+('<label for="'+m+'">'),c=c+("<strong onclick=\"return EVC.HTMLInteraction.showDesc('"+a+"');\">"+f+"</strong>"),c=c+('<span class="desc">'+g+"</span>"),c=c+"</label>",c=c+'<div class="middleColumn">',c=c+('<a href="#'+k+'" class="displayValue" id="'+l+'" onclick="return EVC.HTMLInteraction.showDesc(\''+
a+"');\">"+t+"</a>");EVC.HTMLInteraction.hasRangeSlider&&(c+='<input type="range" tabindex="-1" class="'+e+'" id="'+m+'" oninput="return  EVC.HTMLInteraction.showUpdatedValue(\''+a+"', this);\" onchange=\"return EVC.HTMLInteraction.setValue('"+a+'\', this);" value="'+q+'"  min="'+n+'" max="'+p+'" step="'+r+'" />');c+='<input type="number" inputmode="numeric" pattern="[0-9]*"  class="'+e+'" oninput="return  EVC.HTMLInteraction.startInput(\''+a+'\', this);" id="'+h+'" onclick="return EVC.HTMLInteraction.clickInput(\''+
a+"', this);\" onfocus=\"return EVC.HTMLInteraction.inputReady('"+a+"', this);\" onchange=\"return EVC.HTMLInteraction.setValue('"+a+'\', this);" value="'+q+'" '+d+' min="'+n+'" max="'+p+'" '+u+" />";c+="</div>";EVC.HTMLInteraction.hasGraphs&&(c+='<p class="graphShower"><a href="#'+a+'Chart" onclick="return EVC.HTMLInteraction.showGraphNow(\''+a+"')\">view influence graph \u21e3</a></p>",c+='<figure id="'+a+'Chart" class="chartHolder"><figcaption></figcaption><canvas data-for="'+a+'"></canvas></figure>');
c+="</div>"}return c},getValueFromDefaultsOrSession:function(b,c){var d=EVC.ActualData[b];return c?this.formatValue(b,d):d},formatValue:function(b,c){var d=EVC.DataDescription.keyAssumptionKeys[b];"undefined"==typeof d&&(d=EVC.DataDescription.playAroundAssumptionKeys[b],"undefined"==typeof d&&(d=EVC.DataDescription.otherAssumptionKeys[b],"undefined"==typeof d&&(d="error")));c=parseFloat(c);switch(d){case "number":c=c.formatNumber();break;case "percentage":c=c.formatPercentage();break;case "kmPerDay":c=
c.kmPerYearFormat();break;default:c=c.formatMoney()}return c},clickInput:function(b,c){jQuery(c).removeAttr("readonly");jQuery(c).focus();jQuery(c).select();return!0},inputReady:function(b,c){EVC.HTMLInteraction.showDesc(b)},showUpdatedValue:function(b,c){var d=jQuery(c).val();jQuery("#"+(b+"Field")).val(d);this.startInput(b,c)},startInput:function(b,c){jQuery("#ProfitAndLoss .calcVal").text("calculating ...");this.isTouchScreenTest()&&(jQuery("#ProfitAndLoss p.good").hide(),jQuery("#ProfitAndLoss p.warning").show())},
updateInProgress:!1,setValue:function(b,c,d){!0!==d&&(d=!1);if(this.updateInProgress)console.error("Application can not be updated right now.");else{this.updateInProgress=!0;var a=b+"Field",e=b+"FieldRange",f=b+"Display",g="#"+b+"Holder",k="#"+b+"Holder label strong",h=EVC.DefaultData[b],l="",m=!1;isNaN(c)?(m=EVC.isChanged=!0,d=jQuery(c).val(),d=parseFloat(d.replace(/\$|,/g,"")),isNaN(d)?(d=EVC.ActualData[b],isNaN(d)&&(d=h)):l=jQuery(c).attr("id")):(d&&(m=EVC.isChanged=!0),d=c);d=Math.round(100*d)/
100;c=EVC.DataDescription.labels[b];d!=h?(0<h?jQuery(k).text(c+" (default = "+this.formatValue(b,h)+")"):jQuery(k).text(c),jQuery(g).addClass("changed")):(jQuery(k).text(c),jQuery(g).removeClass("changed"));"kmDrivenPerDay"==b&&3<d/365&&(d=Math.round(d/365),l="");a!=l&&jQuery("#"+a).val(d);e!=l&&jQuery("#"+e).val(d);a=this.formatValue(b,d);jQuery("#"+f).text(a);EVC.serverInteraction.workableLinks()&&jQuery.ajax({method:"GET",url:EVC.serverInteraction.saveLink(),data:{key:b,value:d},cache:!1}).done(function(a){a&&
(EVC.isLocked=!1,EVC.serverKey=a)}).fail(function(a,b){alert("Data could not be saved: "+b)});EVC.ActualData[b]=d;if("kmDrivenPerDay"==b||"yearsBeforeSwitch"==b||"yearsAfterSwitch"==b)"kmDrivenPerDay"==b?EVC.myData.updateKmDrivenPerDay(d):"yearsBeforeSwitch"==b?EVC.myData.updateYearsBeforeSwitch(d):"yearsAfterSwitch"==b&&EVC.myData.updateYearsAfterSwitch(d);m&&this.updateScreen(b);this.updateInProgress=!1}return!1},showDesc:function(b,c){var d=jQuery("div#"+b+"Holder");jQuery(".infocus").each(function(a,
b){jQuery(d).attr("id")!=jQuery(b).attr("id")&&jQuery(b).removeClass("infocus").find("figure").hide()});jQuery(d).hasClass("infocus")||(d.addClass("infocus"),c||jQuery("html, body").animate({scrollTop:jQuery("#"+b+"Holder").offset().top-(jQuery("#ProfitAndLoss").height()+20)},500,function(){jQuery("#"+b+"Holder").find('input[type="number"]').focus();jQuery("#"+b+"Holder").find(".graphShower").show()}));return!1},showGraphNow:function(b){jQuery("#"+b+"Chart").show("slow",function(){EVC.graphMaker.makeGraph(b)}).prev(".graphShower").hide();
